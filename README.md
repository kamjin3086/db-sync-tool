...
本项目是一个基于 Python + Tkinter 的桌面工具，包含两个常用的结构类能力：

- 结构同步：对比源 SQL 与目标库，仅生成“新增表/新增字段”的安全脚本
- 结构导出：将数据库结构导出到 Excel（支持目录索引、合并单 Sheet、表选择）
...
*   **导出为 Excel**：
    - 目录索引（可选）：首个 Sheet 自动生成“库名、服务器、导出时间、表数量”和表清单，点击可跳转到对应表
    - 表清单筛选：默认全选，可一键“全选/全不选”
    - 两种导出模式：
        - 每表一个 Sheet（默认）
        - 合并单 Sheet：在一张 Sheet 内以“表名/表描述/字段表头/字段数据”的块状结构连续写入
...
双击启动程序后，窗口顶部是两个页签：`结构同步` 与 `导出为Excel`。
...
2.  在工具界面中，依次填写信息：
    *   **1. 选择源结构SQL文件**：点击 **“浏览...”** 按钮，选择您刚刚从Navicat导出的 `mydb.sql` 文件。
...
1. 切换到“导出为Excel”页签，填写数据库连接并点击“加载/刷新表列表”（默认全选）。
2. 可选勾选：
   - “生成索引/目录（首个Sheet，内链跳转）”：在首个 Sheet 生成目录，点击表名跳到对应位置。
   - “合并至一张Sheet（按表顺序连续写入）”：将所有表导出到同一张 Sheet。
3. 一键“全选/全不选”切换当前表选择状态；也可逐个取消勾选不想导出的表。
4. 选择保存路径后，点击“导出结构为 Excel”。

> 目录跳转基于 HYPERLINK 公式，兼容 Excel 与 WPS。
...
2.  打开命令行工具 (CMD 或 PowerShell)，安装所有必要的库：
    ```bash
    pip install nuitka zstandard pymysql sqlparse openpyxl
    ```
